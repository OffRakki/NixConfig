...
import { clearWidgetState, exportWidgetState, importWidgetState, renderWidget, setRenderCallback, } from "./clients/widget-state.js";
import { dropStaleFiles, loadSessionState, saveSessionState, sessionStartMode, } from "./clients/session-state-store.js";
import { getDiagnosticTracker } from "./clients/diagnostic-tracker.js";
import { getCascadeSessionStats, getDispatchSlopScoreLine, getLatencyReports, resetDispatchBaselines, } from "./clients/dispatch/integration.js";
import { extractReadPathsFromCommand, extractWrittenPathsFromCommand, } from "./clients/bash-file-access.js";
...
import { loadPiLensGlobalConfig, resolvePiLensFlag, } from "./clients/lens-config.js";
import { initLensEvents } from "./clients/lens-events.js";
import { initLSPConfig } from "./clients/lsp/config.js";
import { getLSPService, resetLSPService } from "./clients/lsp/index.js";
import { EXPANSION_BUDGET_MS, EXPANSION_LIMIT_LINES, tryExpandRead, } from "./clients/read-expansion.js";
import { logReadGuardEvent } from "./clients/read-guard-logger.js";
import { countFileLines, getTouchedLinesForGuard, relocateEditRange, tryCorrectIndentationMismatch, tryCorrectIndentationMismatchFromContent, } from "./clients/read-guard-tool-lines.js";
import { computeTrailingWhitespaceOldTextPatch, findUniqueMatchLineRange, } from "./clients/oldtext-autopatch.js";
...
import { consumeSessionStartGuidance, consumeTestFindings, consumeTurnEndFindings, } from "./clients/runtime-context.js";
import { RuntimeCoordinator } from "./clients/runtime-coordinator.js";
import { handleSessionStart } from "./clients/runtime-session.js";
import { clearLastAnalyzedStateCache, flushDebouncedToolResults, handleToolResult, } from "./clients/runtime-tool-result.js";
...
import { createStarterSemgrepConfig, findLocalSemgrepConfig, loadPiLensSemgrepConfig, removePiLensSemgrepConfig, resolveSemgrepConfig, savePiLensSemgrepConfig, } from "./clients/semgrep-config.js";
...
import { markPiLensLoaded, PI_LENS_LOADED_FROM, } from "./clients/startup-timing.js";
import { getEventLoopStats, shouldLogWorstBlock, startEventLoopMonitor, } from "./clients/event-loop-monitor.js";
...
const _lspConfigInitializedCwds = new Set();
...
export default function (pi) {
    ...
    pi.registerFlag("no-lens", {
        description: "Start pi-lens disabled for this session. Re-enable with /lens-toggle.",
        type: "boolean",
        default: false,
    });
    pi.registerFlag("no-lsp", {
        description: "Disable unified LSP diagnostics and use language-specific fallbacks (for example ts-lsp, pyright)",
        type: "boolean",
        default: false,
    });
    pi.registerFlag("no-autoformat", {
        description: "Disable automatic formatting entirely (deferred format runs at agent_end by default)",
        type: "boolean",
        default: false,
    });
    pi.registerFlag("immediate-format", {
        description: "Run automatic formatting immediately after each write/edit instead of deferring to agent_end",
        type: "boolean",
        default: false,
    });
    ...
    pi.registerFlag("lens-guard", {
        description: "Experimental: block git commit/push when unresolved pi-lens blockers exist",
        type: "boolean",
        default: false,
    });
    pi.registerFlag("lens-semgrep", {
        description: "Enable Semgrep dispatch when a Semgrep config is available (or with --lens-semgrep-config)",
        type: "boolean",
        default: false,
    });
    pi.registerFlag("lens-semgrep-config", {
        description: "Semgrep config for dispatch: local path, auto, p/<pack>, or r/<rule>. Requires --lens-semgrep.",
        type: "string",
        default: "",
    });
    ...
    let contextInjectionEnabled = process.env.PI_LENS_NO_CONTEXT_INJECTION !== "1" &&
    ...
    function mountLensWidget(ui) {
        ...
        setWidget("pi-lens", (tui, theme) => {
            ...
            return {
                render: (width) => renderWidget(width, theme),
    ...
    pi.registerCommand("lens-toggle", {
        ...
        handler: async (_args, ctx) => {
            ...
            ctx.ui.notify(lensEnabled
                ? "pi-lens enabled for this session."
                : "pi-lens disabled for this session. Run /lens-toggle again to resume.", lensEnabled ? "info" : "warning");
        },
    });
    pi.registerCommand("lens-context-toggle", {
        ...
        handler: async (_args, ctx) => {
            ...
            ctx.ui.notify(contextInjectionEnabled
                ? "pi-lens context injection enabled — findings will be added to the next turn."
                : "pi-lens context injection disabled — findings are still cached (lens_diagnostics, /lens-health) but not added to model context.", contextInjectionEnabled ? "info" : "warning");
        },
    });
    pi.registerCommand("lens-widget-toggle", {
        ...
        handler: async (_args, ctx) => {
            ...
            ctx.ui.notify(lensWidgetVisible
                ? "pi-lens widget shown. Run /lens-widget-toggle to hide it."
                : "pi-lens widget hidden. Run /lens-widget-toggle to show it.", "info");
        },
    });
    pi.registerCommand("lens-semgrep", {
        description: "Manage Semgrep dispatch. Usage: /lens-semgrep status | enable [--config <auto|p/pack|path>] | disable | init",
        handler: async (args, ctx) => {
            const parts = normalizeCommandArgs(args);
            const action = parts[0] ?? "status";
            const cwd = ctx.cwd ?? runtime.projectRoot;
            function readConfigArg() {
                const flagIndex = parts.findIndex((part) => part === "--config" || part === "-c");
                if (flagIndex >= 0)
            ...
            if (action === "enable") {
                ...
                ctx.ui.notify(`Semgrep dispatch enabled (${config ? `config: ${config}` : `local config: ${localConfig}`}). Saved ${savedPath}`, "info");
                return;
            }
            if (action === "disable") {
            ...
            if (action === "clear") {
                ...
                ctx.ui.notify(removed
                    ...
                    : "No .pi-lens/semgrep.json found.", "info");
                return;
            }
            if (action === "init") {
                const configPath = createStarterSemgrepConfig(cwd);
                const savedPath = savePiLensSemgrepConfig(cwd, { enabled: true });
                ctx.ui.notify(`Created starter Semgrep config at ${configPath} and enabled Semgrep dispatch (${savedPath}).`, "info");
                return;
            }
            if (action !== "status") {
                ctx.ui.notify("Usage: /lens-semgrep status | enable [--config <auto|p/pack|path>] | disable | clear | init", "warning");
                return;
            }
            const localConfig = findLocalSemgrepConfig(cwd);
            ...
            lines.push("", "No auto-install. Token/login is only needed for Semgrep AppSec/Pro/managed configs; local .semgrep.yml scans do not require a token.");
    ...
    pi.registerCommand("lens-booboo", {
        ...
        handler: async (args, ctx) => {
            const { complexityClient, todoScanner, knipClient, jscpdClient, typeCoverageClient, depChecker, } = await loadBootstrapClients();
            return handleBooboo(args, ctx, {
    ...
    pi.registerCommand("lens-health", {
        ...
        handler: async (_args, ctx) => {
            ...
            if (last) {
                lines.push("", `Last dispatch: ${path.basename(last.filePath)} (${last.totalDurationMs}ms, ${last.totalDiagnostics} diagnostics)`);
            ...
            if (elStats) {
                lines.push("", `Event loop (session): worst block ${elStats.maxMs}ms · p99 ${elStats.p99Ms}ms · mean ${elStats.meanMs}ms`);
            ...
            if (diagStats.repeatOffenders.length > 0) {
                ...
                for (const offender of diagStats.repeatOffenders.slice(0, 5)) {
                    lines.push(`  ${path.basename(offender.filePath)}:${offender.line} ${offender.ruleId} (${offender.count}x)`);
                }
            }
            if (diagStats.topViolations.length > 0) {
                ...
                for (const v of diagStats.topViolations.slice(0, 5)) {
                    const samplePath = v.samplePaths.length > 0
                        ? path
    ...
    pi.registerCommand("lens-tools", {
        ...
        handler: async (_args, ctx) => {
            ...
            if (bySource["npx-fallback"].length > 0) {
                lines.push("", `📦 npx fallback (${bySource["npx-fallback"].length} - on-demand install):`);
    ...
    for (const tool of [
        ...
        createLensDiagnosticsTool(cacheManager, () => runtime.projectRoot, undefined, 
    ...
    pi.on("session_start", async (event, ctx) => {
        try {
            ...
            const { metricsClient, todoScanner, biomeClient, ruffClient, knipClient, jscpdClient, govulncheckClient, gitleaksClient, typeCoverageClient, depChecker, testRunnerClient, goClient, rustClient, } = await loadBootstrapClients();
            await handleSessionStart({
                ...
                goClient,
                rustClient,
                ensureTool: async (name) => (await import("./clients/installer/index.js")).ensureTool(name),
            ...
            if (startMode === "fork" && pendingForkSnapshot) {
                ...
                if (stableSessionId) {
                    void saveSessionState(ctx.cwd ?? process.cwd(), stableSessionId, exportWidgetState());
                }
                dbg(`session_start: fork — branched ${forkedFileCount} file(s) from source`);
            }
                ...
                if (stableSessionId) {
                    ...
                    if (persisted?.widget) {
                        ...
                        dbg(`session_start: ${reasonLabel} ${stableSessionId} — rehydrated ${fresh.files.length} file(s)` +
    ...
    pi.on("session_before_fork", () => {
        try {
            ...
            dbg(`session_before_fork: stashed ${pendingForkSnapshot.files.length} file(s) for the fork`);
        }
        catch (forkErr) {
    ...
    pi.on("tool_call", async (event, ctx) => {
        ...
        if (!isExternalOrVendor &&
            ...
            if (baseline) {
                ...
                const { captureSnapshot } = await import("./clients/metrics-history.js");
    ...
    pi.on("turn_end", async (_event, ctx) => {
        ...
        try {
            ...
            await flushDebouncedToolResults();
            const { knipClient, depChecker, testRunnerClient } = await loadBootstrapClients();
            await handleTurnEnd({
                ctxCwd: ctx.cwd,
                getFlag: (name) => getLensFlag(name),
                dbg,
            ...
            if (runtime.hasStableSessionId) {
                void saveSessionState(ctx.cwd ?? process.cwd(), runtime.telemetrySessionId, exportWidgetState());
            }
        }
        catch (turnEndErr) {
    ...
    pi.on("context", async (event, ctx) => {
        if (!lensEnabled || !contextInjectionEnabled)
            return;
        try {
