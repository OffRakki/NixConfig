...
import { analyzeFile, analyzeFileFresh, createMcpHost, diagnosticStats, ensureLspConfig, ipcPathForCwd, lspStatus, projectScan, recentLatency, resolveRebuildScript, runRebuild, runSessionStart, runTurnEnd, summarizeScan, symbolImpact, symbolSearch, } from "../clients/lens-engine.js";
...
function maybeAutoSessionStart() {
    if (autoSessionFired || process.env.PI_LENS_MCP_AUTO_SESSION !== "1")
        return;
    ...
    void ensureReady(DEFAULT_CWD)
        ...
        .catch((err) => console.error(`[pi-lens-mcp] auto session_start failed: ${err}`));
}
...
function toolText(summary, structured) {
...
const TOOLS = [
    ...
    {
        name: "pilens_impact",
        description: "Transitive, depth-bounded impact of a file over the review graph: what " +
    ...
    },
    {
        ...
        description: "LSP code navigation: definition, references, hover, documentSymbol, " +
...
function formatAnalyze(result, cwd, mode) {
...
async function callTool(name, args) {
    ...
    if (name === "pilens_symbol_search") {
        ...
        if (!available) {
            return toolText("No word index for this workspace yet — run pilens_session_start first.", { available: false, query });
        }
        if (results.length === 0) {
    ...
    if (name === "pilens_impact") {
        ...
        if (!file.trim())
            return toolText("Provide a `file`.");
        const cwd = typeof args.cwd === "string" ? args.cwd : DEFAULT_CWD;
        const maxDepth = typeof args.maxDepth === "number" && Number.isFinite(args.maxDepth)
            ? Math.max(1, Math.floor(args.maxDepth))
            : undefined;
        const maxHits = typeof args.maxHits === "number" && Number.isFinite(args.maxHits)
            ? Math.max(1, Math.floor(args.maxHits))
            : undefined;
        const { available, hits, truncated, maxDepthReached } = await symbolImpact(file, cwd, { maxDepth, maxHits });
        const rel = (f) => (f ? path.relative(cwd, f) : "(unknown)");
        if (!available) {
            return toolText("Review graph is empty — run pilens_session_start first.", { available: false });
        }
        if (hits.length === 0) {
        ...
        const lines = [
            ...
            ...byDepth
                .slice(0, 40)
                .map((hit) => `  d${hit.depth} ${hit.relation} ← ${hit.symbol ? `${hit.symbol} ` : ""}${rel(hit.file)}`),
        ];
        return toolText(lines.join("\n"), {
            file,
            truncated,
