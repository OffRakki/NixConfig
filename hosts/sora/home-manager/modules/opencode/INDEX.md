...
| `hosts/sora/home-manager/modules/fish/fish.nix` | fish, tide-plugin, direnv, zoxide, fastfetch, color-scheme, aliases(cp,rsync,mv,rm,df,du,tree,fzf,grep), abbrs(jj,nix,opencode,yt-dl) |
...
- **sops/secrets** → `.sops.yaml`, `secrets.yaml`, `configuration.nix`(both), `private.yaml`, `calendar.nix`, `syncthing.nix`, `onedrive.nix`, `ciel.nix`
...
hosts/sora/nixos/configuration.nix
  ...
  → sops: secrets.yaml, private.yaml
...
*Last updated: 2026-06-17 by Ciel. This file lives at the NixConfig root.*
