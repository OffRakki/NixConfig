...
function getPiLensMetadata(metadata) {
    const nested = metadata["pi-lens"] ?? metadata.pi_lens;
    return nested && typeof nested === "object"
        ? nested
        : {};
}
function metadataString(metadata, piLens, key) {
    const direct = piLens[key] ?? metadata[`pi_lens_${key}`];
    return typeof direct === "string" && direct.trim()
        ? direct.trim()
        : undefined;
}
function metadataBoolean(metadata, piLens, key) {
    return piLens[key] === true || metadata[`pi_lens_${key}`] === true;
}
function normalizeDefectClass(value) {
    if (!value)
        return undefined;
    const normalized = value.toLowerCase().replace(/_/g, "-");
    if (normalized === "silent-error" ||
...
function semgrepSemantic(result, defectClass) {
    ...
    const confidence = String(metadata.confidence ??
        metadata.semgrep_confidence ??
        piLens.confidence ??
        "").toLowerCase();
    const highSignalSecurity = defectClass === "injection" ||
        defectClass === "secrets" ||
        defectClass === "safety";
...
function mapSeverity(semgrepSeverity, semantic) {
...
function parseSemgrepJson(raw, ctx) {
    if (!raw.trim())
        return [];
    let parsed;
    try {
    ...
    for (const [index, result] of results.entries()) {
        ...
        const metadata = result.extra?.metadata ?? {};
        const piLens = getPiLensMetadata(metadata);
        const explicitDefect = normalizeDefectClass(metadataString(metadata, piLens, "defect_class"));
        const defectClass = explicitDefect ?? classifyDefect(rule, "semgrep", message);
        ...
        const fixSuggestion = metadataString(metadata, piLens, "fix") ??
        ...
        diagnostics.push({
            ...
            fixKind: fixSuggestion || result.extra?.fix_regex ? "suggestion" : undefined,
            fixSuggestion,
        });
    }
    return diagnostics;
...
const semgrepRunner = {
    ...
    async run(ctx) {
        ...
        if (!(await (semgrep.isAvailableAsync(cwd)))) {
            return { status: "skipped", diagnostics: [], semantic: "none" };
        }
        const cmd = semgrep.getCommand(cwd) ?? "semgrep";
        const args = ["scan", "--json", "--metrics=off", "--timeout", "5"];
        if (resolved.configArg)
            args.push("--config", resolved.configArg);
