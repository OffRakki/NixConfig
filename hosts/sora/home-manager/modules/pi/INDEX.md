...
| `hosts/sora/home-manager/modules/pi/pi.nix` | pi-coding-agent-enable, settings(deepseek-provider,openai,model,compaction,retry,branchSummary,treeFilterMode,terminal,images,theme(gruvbox-dark-hard)), models.json, home.file(extensions,skills-symlinks,prompts,themes(catppuccin-mocha,ciel-cursor,gruvbox-dark-hard),keybindings,mcp.json(obsidian-mcp -> /home/rakki/sync/geral/Obsidian/{Main,Summaries}),APPEND_SYSTEM.md), packages(pi-drawio,pi-intercom,pi-lean-ctx,pi-lens,pi-simplify,pi-namespace,pi-ask-user,pi-web-access,pi-mcp-adapter,pi-markdown-preview,pi-powerline-footer,pi-hermes-memory,pi-invisible-continue,pi-subagents,rpiv-pi,rpiv-todo,rpiv-args,rpiv-ask-user-question,piolium; commented optional: pi-chrome, pi-sketch, rpiv-btw, rpiv-i18n, rpiv-advisor, rpiv-workflow), sops-secrets(deepseek,openai,lucky-info,skill-firefly,skill-lumis,webSearchJson), persistence(.pi,.local/share/pi,.pi-lens,.config/lean-ctx,.local/share/lean-ctx), xdg.configFile(lean-ctx/config.toml) |
