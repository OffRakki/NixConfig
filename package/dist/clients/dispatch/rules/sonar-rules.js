...
function makeD(_id, rule, filePath, line, col, message) {
    return {
...
export const commentedOutCodeRule = {
    ...
    evaluate(ctx, store) {
        ...
        for (const r of ranges) {
            ...
            diagnostics.push(makeD("commented-out-code", "commented-out-code", ctx.filePath, line + 1, 1, "Commented-out code detected — remove or restore it"));
        }
        return diagnostics;
    },
};
...
export const duplicateStringLiteralRule = {
    ...
    evaluate(ctx, store) {
        ...
        function isSkipped(s) {
            if (SKIP_STRINGS.has(s.toLowerCase()))
                return true;
        ...
        for (const [val, { count, line }] of counts) {
            if (count >= MIN_DUPLICATES) {
                diagnostics.push(makeD("duplicate-string-literal", "duplicate-string-literal", ctx.filePath, line, 1, `String literal "${val}" repeated ${count} times — extract to a named constant`));
            }
        }
        return diagnostics;
    },
...
export const functionInLoopRule = {
    id: "function-in-loop",
    requires: ["file.content"],
    appliesTo: tsFile,
    evaluate(ctx, store) {
            ...
            if (isFn && isInsideLoop(node)) {
                ...
                const label = ts.isFunctionDeclaration(node)
                    ...
                    : ts.isFunctionExpression(node)
                        ? "Function expression"
                        : "Arrow function";
                diagnostics.push(makeD("function-in-loop", "function-in-loop", ctx.filePath, line + 1, character + 1, `${label} inside a loop — creates a new function object on every iteration`));
            }
            ts.forEachChild(node, visit);
        }
        visit(sf);
...
export const corsWildcardRule = {
    ...
    evaluate(ctx, store) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (isWildcard) {
                diagnostics.push({
                    ...makeD("cors-wildcard", "cors-wildcard", ctx.filePath, i + 1, 1, "CORS wildcard origin (*) allows any website to make credentialed requests — restrict to known origins"),
                    severity: "error",
                    semantic: "blocking",
                });
            }
...
function isEscapeRegExpCall(expr) {
    if (!ts.isCallExpression(expr))
        return false;
    const callee = expr.expression;
    return ((ts.isIdentifier(callee) && callee.text === "escapeRegExp") ||
        (ts.isPropertyAccessExpression(callee) &&
            callee.name.text === "escapeRegExp"));
}
function hasParameterShadow(node, name) {
...
function findVisibleVariableInitializer(sf, useNode, name) {
    if (hasParameterShadow(useNode, name))
        return undefined;
    let best;
...
function isEscapedRegExpArgument(expr, sf, seen = new Set()) {
    ...
    if (isEscapeRegExpCall(expr))
        return true;
    if (ts.isTemplateExpression(expr)) {
        return expr.templateSpans.every((span) => isEscapedRegExpArgument(span.expression, sf, seen));
    }
    if (ts.isBinaryExpression(expr) &&
        expr.operatorToken.kind === ts.SyntaxKind.PlusToken) {
        return (isEscapedRegExpArgument(expr.left, sf, seen) &&
...
export const dynamicRegexpRule = {
    id: "dynamic-regexp",
    requires: ["file.content"],
    appliesTo: tsFile,
    evaluate(ctx, store) {
        ...
        function visit(node) {
            if (ts.isNewExpression(node) &&
                ...
                if (!isEscapedRegExpArgument(firstArg, sf)) {
                    const { line, character } = sf.getLineAndCharacterOfPosition(node.getStart(sf));
                    diagnostics.push(makeD("dynamic-regexp", "dynamic-regexp", ctx.filePath, line + 1, character + 1, "new RegExp() with dynamic argument — potential ReDoS if argument comes from user input"));
                }
            }
            ts.forEachChild(node, visit);
        }
...
export const maxSwitchCasesRule = {
    id: "max-switch-cases",
    requires: ["file.content"],
    appliesTo: tsFile,
    evaluate(ctx, store) {
        ...
        function visit(node) {
            if (ts.isSwitchStatement(node)) {
                ...
                if (caseCount > MAX_SWITCH_CASES) {
                    const { line, character } = sf.getLineAndCharacterOfPosition(node.getStart(sf));
                    diagnostics.push(makeD("max-switch-cases", "max-switch-cases", ctx.filePath, line + 1, character + 1, `Switch has ${caseCount} cases (max ${MAX_SWITCH_CASES}) — consider a lookup map or polymorphism`));
                }
            }
            ts.forEachChild(node, visit);
        }
    ...
    evaluate(ctx, store) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (const p of CREDENTIAL_PATTERNS) {
                if (p.test(line)) {
                    diagnostics.push({
                        ...makeD("no-commented-credentials", "no-commented-credentials", ctx.filePath, i + 1, 1, "Possible credential in commented-out code — remove it and rotate the secret"),
                        severity: "error",
                        semantic: "blocking",
                    });
                    break;
