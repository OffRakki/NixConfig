#!/usr/bin/env python3
"""
llm-suggest-lsp — LSP server that surfaces LLM suggested edits as editor diagnostics
with code actions to apply the replacement.

Reads suggestions from $XDG_STATE_HOME/llm-suggest/suggestions.json (written by
Pi's suggest_edit tool), watches for file changes, and publishes diagnostics
with quickfix code actions.
"""
...
def path_to_uri(path_str: str) -> str:
    return pathlib.Path(path_str).resolve().as_uri()
...
def normalize_range(item: dict) -> dict:
    start = item.get("range", {}).get("start", {})
    end = item.get("range", {}).get("end", {})
    start_line = _safe_int(start.get("line"), 0)
    start_char = _safe_int(start.get("character"), 0)
    end_line = _safe_int(end.get("line"), start_line)
    end_char = _safe_int(end.get("character"), start_char)

    if item.get("wholeLine"):
        return {
...
def publish(uri: str) -> None:
    path_str = uri_to_path(uri)
    if not path_str:
        return
    matches = suggestions_for(path_str)
    log(f"publishing {len(matches)} diagnostic(s) for {path_str}")
...
def position_key(position: dict) -> tuple[int, int]:
    return (_safe_int(position.get("line")), _safe_int(position.get("character")))


def request_matches_range(request_range: dict | None, action_range: dict) -> bool:
...
def context_diagnostic_ids(params: dict) -> set[str] | None:
...
def code_actions(params: dict) -> list[dict]:
    uri = params.get("textDocument", {}).get("uri")
    path_str = uri_to_path(uri) if uri else None
    if not path_str:
        return []
    actions: list[dict] = []
    request_range = params.get("range")
    diagnostic_ids = context_diagnostic_ids(params)
    for item in suggestions_for(path_str):
...
def handle(msg: dict) -> None:
    ...
    if method == "initialize":
        ...
        if params.get("command") == "llm-suggest.clearSuggestion":
            ...
            if args:
                # clear_suggestion defined below
                _clear_suggestion(args[0])
        respond(req_id, None)
...
def write_suggestions(items: list) -> None:
...
def _clear_suggestion(suggestion_id: str) -> None:
...
def reader() -> None:
    while True:
        ...
        length = _safe_int(headers.get("content-length"), 0)
...
def poller() -> None:
    ...
    while not shutdown:
        try:
            stat_result = SUGGESTIONS_FILE.stat()
            payload = stat_result.st_mtime_ns
        except FileNotFoundError:
            payload = None
