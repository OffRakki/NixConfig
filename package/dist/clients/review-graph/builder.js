...
import { functionFactProvider, } from "../dispatch/facts/function-facts.js";
import { importFactProvider, } from "../dispatch/facts/import-facts.js";
...
function changedSignatureFiles(previous, next) {
...
function loadPersistedGraph(cwd) {
...
function localImportToFile(cwd, filePath, source) {
...
async function ensureTsFacts(filePath, cwd, facts) {
...
function addJsTsFile(graph, cwd, filePath, facts) {
    const normalized = normalizeMapKey(filePath);
...
function addTreeSitterFile(graph, filePath, languageId, extracted) {
    const normalized = normalizeMapKey(filePath);
    const fileNodeId = `file:${normalized}`;
    addNode(graph, {
        id: fileNodeId,
...
function addCxxIncludeEdges(graph, cwd, filePath) {
    let content = "";
    try {
...
async function addFileToGraph(graph, cwd, file, facts) {
    const kind = detectFileKind(file);
    ...
    if (kind === "cxx")
        addCxxIncludeEdges(graph, cwd, file);
}
...
async function updateGraphFiles(graph, cwd, files, facts) {
...
async function _doBuildGraph(cwd, changedFiles, facts) {
    ...
    if (memCached && memChanged !== undefined && memChanged.length > 0) {
        ...
        const { trulyChanged, hashes } = await confirmContentChanged(memChanged, memCached.fileHashes);
    ...
    const diskChanged = diskCached && diskCached.fileSignatures.size > 0
    ...
    if (diskCached && diskChanged !== undefined && diskChanged.length > 0) {
        ...
        const { trulyChanged, hashes } = await confirmContentChanged(diskChanged, diskCached.fileHashes);
        if (trulyChanged.length === 0) {
            const graph = cloneGraph(diskCached.graph);
            rebuildIndexes(graph);
            graph.changedSymbolsByFile.clear();
...
export function buildOrUpdateGraph(cwd, changedFiles, facts) {
