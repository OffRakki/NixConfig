...
import { getKnipIgnorePatterns, getProjectIgnoreGlobs, isTestFile, } from "../clients/file-utils.js";
import { validateProductionReadiness } from "../clients/production-readiness.js";
import { detectProjectMetadata, getAvailableCommands, } from "../clients/project-metadata.js";
import { RunnerTracker } from "../clients/runner-tracker.js";
import { safeSpawn } from "../clients/safe-spawn.js";
import { collectSourceFiles, getFilterStats, } from "../clients/source-filter.js";
...
let _sharedTreeSitterClient = null;
...
export async function handleBooboo(args, ctx, clients, pi) {
    ...
    const categoryKey = (name) => name.toLowerCase().replace(/\s+/g, "-");
    ...
    if (filterStats.skipped > 0) {
        ...
        console.log(`[lens-booboo] Filtered ${filterStats.skipped} build artifacts (${byTypeStr}), scanning ${filterStats.kept} source files`);
    }
    ...
    await tracker.run("ast-grep (design smells)", async () => {
        ...
        try {
            ...
            if (output.trim() && result.status !== undefined) {
                ...
                const filteredIssues = issues.filter((issue) => !isFalsePositive(categoryKey("ast-grep"), issue.file, issue.line));
    ...
    await tracker.run("tree-sitter patterns", async () => {
        ...
        for (const filePath of allFiles) {
            ...
            if (!langId)
                continue;
            const langQueries = allQueries.filter((q) => q.language === langId ||
    ...
    await tracker.run("fact rules", async () => {
        ...
        for (const filePath of tsFiles) {
            ...
            const diagnostics = evaluateRules(ctx).filter((d) => FACT_SEVERITY_FILTER.has(d.severity ?? "warning"));
    ...
    await tracker.run("dead code (Knip)", async () => {
        ...
        const knipResult = await clients.knip.analyze(targetPath, getKnipIgnorePatterns());
    ...
    await tracker.run("duplicate code (jscpd)", async () => {
        ...
        const jscpdResult = await clients.jscpd.scan(targetPath, 5, 50, isTsProject);
    ...
    await tracker.run("compiler checks", async () => {
        ...
        if (langs.has("rust") &&
            ...
            for (const line of output.split("\n")) {
                ...
                try {
                    ...
                    if (!inner || !["error", "warning"].includes(inner.level))
                        continue;
                    const span = inner.spans?.find((s) => s.is_primary) ??
                        inner.spans?.[0];
                    if (!span)
                        continue;
            ...
            if (hasPyright) {
                ...
                try {
                    ...
                    for (const diag of json?.generalDiagnostics ?? []) {
                        if (!["error", "warning"].includes(diag.severity))
                            continue;
        ...
        if (langs.has("ruby") &&
            ...
            if (hasRubocop) {
                const result = safeSpawn("rubocop", [
                    "--format",
                    "json",
                    "--no-color",
        ...
        if (nodeFs.existsSync(path.join(targetPath, "pom.xml"))) {
        ...
        else if (nodeFs.existsSync(path.join(targetPath, "build.gradle")) ||
            ...
            const output = (result.stdout || "") + (result.stderr || "");
            const gradleRe = /^([^\s:]+\.(?:java|kt)):\s*(\d+):\s*(?:error|warning):\s+([^\n]+)/gm;
        ...
        if (nodeFs
            ...
            if (hasDotnet) {
                ...
                const output = (result.stdout || "") + (result.stderr || "");
                const csRe = /^([^\s(]+\.cs)\((\d+),(\d+)\):\s+(error|warning)\s+([A-Z]+\d+):\s+([^[]+)/gm;
        ...
        for (const issue of issues) {
            if (!byCompiler[issue.compiler])
                byCompiler[issue.compiler] = [];
    ...
    if (projectMeta.testFramework)
        tools.push(`🧪 ${projectMeta.testFramework}`);
    else if (projectMeta.hasTests)
        tools.push("🧪 tests");
    if (projectMeta.linter)
        tools.push(`🔍 ${projectMeta.linter}`);
    if (projectMeta.formatter)
        tools.push(`✨ ${projectMeta.formatter}`);
    if (tools.length > 0)
        projectSection += `\n**Tools:** ${tools.join(" | ")}`;
    // Available commands
...
${fullReport.join("\n")}`;
    ...
    if (summaryItems.length === 0) {
        ...
        const runnerLines = tracker
            ...
            .map((r) => `  ${r.status === "error" ? "✗" : "⚠"} ${r.name}: ${r.findings} finding${r.findings !== 1 ? "s" : ""} (${formatElapsed(r.elapsedMs)})`);
        const summaryLines = [
