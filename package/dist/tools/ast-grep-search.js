...
export function createAstGrepSearchTool(astGrepClient) {
    return {
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const { pattern, paths, selector, context, skip, strictness, rule, insideKind, hasKind, follows, precedes } = params;
            ...
            if (!effectiveRule && hasStructuralIntent({ insideKind, hasKind, follows, precedes })) {
                try {
                    ...
                    return {
                        content: [{ type: "text", text: `Error synthesizing rule: ${err}` }],
                        isError: true,
                        details: {},
                    };
                }
            ...
            if (effectiveRule && effectiveRule.trim().length > 0) {
                ...
                if (ruleResult.error) {
                    ...
                    return {
                        content: [
                            { type: "text", text: errorTextWithHint(ruleResult.error) },
                        ],
                        isError: true,
                        details: {},
                    };
                ...
                return {
                    content: [{ type: "text", text: `${output}${paginationNote}` }],
                    details: {
                        matchCount: page.length,
