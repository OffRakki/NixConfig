 ...
 */
async function collectWorkspaceDiagnosticFiles(root) {
    const files = [];
    async function walk(current) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (!WORKSPACE_DIAGNOSTICS_SKIP_DIRS.has(entry.name))
                    await walk(full);
            }
...
export class LSPService {
    ...
    async touchFile(filePath, content, options = {}) {
        ...
        if (diagnosticsMode !== "none") {
            ...
            if (envWait !== undefined) {
                ...
                const strategyWait = getStrategy(spawned[0].client.serverId).aggregateWaitMs;
     ...
     */
    async runWorkspaceDiagnostics(cwd) {
        const startedAt = Date.now();
 ...
 */
export function __collectWorkspaceDiagnosticFilesForTest(root) {
