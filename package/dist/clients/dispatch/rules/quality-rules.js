...
function isExpectedNumericContext(node) {
    const parent = node.parent;
    if (!parent)
        return false;
    const pk = parent.kind;
    // const X = 42  /  let X = 42  /  var X = 42
    if (pk === ts.SyntaxKind.VariableDeclaration)
        return true;
    // enum Member = 42
    if (pk === ts.SyntaxKind.EnumMember)
        return true;
    // object property: { timeout: 5000 }
    if (pk === ts.SyntaxKind.PropertyAssignment)
        return true;
    // default parameter: fn(x = 5)
    if (pk === ts.SyntaxKind.Parameter)
        return true;
    // type literal: foo: 42
    if (pk === ts.SyntaxKind.PropertyDeclaration)
        return true;
    // array index access: arr[2]
    if (pk === ts.SyntaxKind.ElementAccessExpression)
        return true;
    // export const X = 42
    if (pk === ts.SyntaxKind.ExportAssignment)
        return true;
    ...
    if (pk === ts.SyntaxKind.PrefixUnaryExpression) {
        ...
        if (!gp)
            return false;
        const gpk = gp.kind;
        return (gpk === ts.SyntaxKind.VariableDeclaration ||
        ...
        function visit(node) {
            if (ts.isNumericLiteral(node)) {
                ...
                if (!MAGIC_NUMBER_ALLOWLIST.has(val) && !isExpectedNumericContext(node)) {
                    ...
                    diagnostics.push(makeD("no-magic-numbers", ctx.filePath, line + 1, character + 1, `Magic number ${node.text} — extract to a named constant`));
                }
            }
...
export const noBooleanParamsRule = {
    ...
    evaluate(ctx, store) {
        ...
        function checkParams(params) {
            for (const param of params) {
                ...
                if (BOOLEAN_PREFIX_OK.test(name) ||
                    BOOLEAN_SUFFIX_OK.test(name) ||
                    BOOLEAN_WHOLE_OK.test(name) ||
                    name.startsWith("_"))
                    continue;
                const isBoolean = param.type.kind === ts.SyntaxKind.BooleanKeyword ||
                    (ts.isUnionTypeNode(param.type) &&
                        param.type.types.every((t) => t.kind === ts.SyntaxKind.BooleanKeyword ||
                ...
                diagnostics.push(makeD("no-boolean-params", ctx.filePath, line + 1, character + 1, `Boolean parameter '${name || "?"}' — use a descriptive options object or string enum instead`));
            }
        }
        function visit(node) {
...
export const highImportCouplingRule = {
    ...
    evaluate(ctx, store) {
        ...
        if (count <= IMPORT_COUPLING_THRESHOLD)
            return [];
        return [
            makeD("high-import-coupling", ctx.filePath, 1, 1, `File imports from ${count} distinct modules (threshold: ${IMPORT_COUPLING_THRESHOLD}) — split responsibilities`),
        ];
    },
};
// ---------- QR-004: no-complex-conditionals ----------
const MAX_LOGICAL_OPS = 4;
function countLogicalOps(node) {
    let count = 0;
    function walk(n) {
...
export const noComplexConditionalsRule = {
    ...
    evaluate(ctx, store) {
        ...
        function visit(node) {
            ...
            if (ts.isIfStatement(node))
                ...
                condNode = node.condition;
            else if (ts.isConditionalExpression(node))
                condNode = node.condition;
            if (condNode) {
                const ops = countLogicalOps(condNode);
                if (ops > MAX_LOGICAL_OPS) {
                    const { line, character } = sf.getLineAndCharacterOfPosition(condNode.getStart(sf));
                    diagnostics.push(makeD("no-complex-conditionals", ctx.filePath, line + 1, character + 1, `Condition has ${ops} logical operators (max: ${MAX_LOGICAL_OPS}) — extract sub-conditions to named variables`));
                }
            }
            ts.forEachChild(node, visit);
        }
...
export const highEntropyStringRule = {
        ...
        function visit(node) {
            if (ts.isStringLiteral(node)) {
                ...
                if (h >= ENTROPY_THRESHOLD) {
                    const { line, character } = sf.getLineAndCharacterOfPosition(node.getStart(sf));
                    diagnostics.push({
                        ...makeD("high-entropy-string", ctx.filePath, line + 1, character + 1, `High-entropy string literal (entropy ${h.toFixed(2)}) — possible hardcoded secret`),
                        severity: "error",
                        semantic: "blocking",
                    });
                }
