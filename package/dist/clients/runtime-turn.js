...
function scheduleLSPIdleReset(resetFn, delayMs) {
...
export async function handleTurnEnd(deps) {
    const { ctxCwd, getFlag, dbg, runtime, cacheManager, knipClient, depChecker, testRunnerClient, resetLSPService, resetFormatService, } = deps;
        ...
        if (previousFailedHard) {
            ...
            if (knipResult.success && knipResult.issues.length > 0) {
                ...
                if (unusedExportIssues.length > 0) {
                    let report = "⚠️ Newly unused exports in modified files — check if callers need updating (Knip):\n";
    ...
    const gitleaksCacheEntry = cacheManager.readCache("gitleaks", cwd);
    if (gitleaksCacheEntry?.data?.findings?.length) {
        const findings = gitleaksCacheEntry.data.findings.slice(0, 5);
        let report = "🔴 STOP — committed secrets detected (gitleaks). Rotate the credentials and remove from source:\n";
        for (const f of findings) {
            ...
            for (const file of madgeFiles) {
                ...
                if (depResult.hasCircular && depResult.circular.length > 0) {
                    const circularDeps = depResult.circular
                        .flatMap((d) => d.path)
                        .filter((p) => !absPath.endsWith(path.basename(p)));
    ...
    if (!getFlag("no-tests") && files.length > 0) {
        ...
        for (const file of files) {
            ...
            if (target && !seen.has(target.testFile)) {
                ...
                targets.push(target);
                dbg(`turn_end: ${file} → test ${target.runner} ${path.relative(cwd, target.testFile)} (${target.strategy})`);
            }
            else if (!target) {
