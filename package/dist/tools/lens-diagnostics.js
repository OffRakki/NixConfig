...
import { loadProjectDiagnosticsDeltaReport, loadProjectDiagnosticsSnapshot, } from "../clients/project-diagnostics/cache.js";
    ...
    return {
        ...
        parameters: Type.Object({
            ...
            refreshRunners: Type.Optional(Type.Union([
                ...
                description: "mode=full only: false/none = LSP + widget state only. cached = include cached project-runner snapshot. cheap = refresh tree-sitter + fact-rules first. all currently aliases cheap and is reserved for future heavyweight runners.",
            })),
            maxProjectFiles: Type.Optional(Type.Number({
                description: "mode=full refreshRunners=cheap/all only: cap project files scanned by cheap runners.",
            })),
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const maxProjectFiles = typeof params.maxProjectFiles === "number" &&
...
function appendProjectDiagnosticsDeltaLines(lines, cwd, report, severity) {
    const diagnostics = (report?.diagnostics ?? []).filter((diagnostic) => matchesSeverity(projectDiagnosticToWidget(diagnostic), severity));
...
function formatDeltaMode(cacheManager, cwd, severity) {
    ...
    const projectDeltaCount = appendProjectDiagnosticsDeltaLines(lines, cwd, projectDelta, severity);
    ...
    if (lines.length === 0) {
        ...
        const carried = getFileDiagnosticSummaries().filter((f) => f.diagnostics.length > 0);
...
async function formatFullMode(cwd, severity, lspService, options = {}) {
    ...
    const projectDelta = loadProjectDiagnosticsDeltaReport(cwd);
    const summaries = mergeDiagnosticsWithWidgetSummaries(getFileDiagnosticSummaries(), lspResults, projectSnapshot, projectDelta);
...
function formatAllMode(cwd, severity, summaries = getFileDiagnosticSummaries(), detailOverrides = { mode: "all" }, staleDropped = 0) {
