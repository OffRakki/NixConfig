...
function scheduleStartupScans(deps, runtime, sessionGeneration, analysisRoot, snapshotRoot, languageProfile, dbg) {
    const { todoScanner, cacheManager, knipClient, jscpdClient, govulncheckClient, gitleaksClient, astGrepClient, } = deps;
    ...
    runTask("word-index", async () => {
        ...
        saveRuntimeProjectSnapshot({ cwd: snapshotRoot, runtime, dbg });
        dbg(`session_start word-index: ${runtime.wordIndex.docCount} files, ` +
...
export async function handleSessionStart(deps) {
    ...
    const { ctxCwd, getFlag, notify, dbg, log, runtime, metricsClient, cacheManager, typeCoverageClient: _typeCoverageClient, testRunnerClient, goClient, rustClient, ensureTool, cleanStaleTsBuildInfo, resetDispatchBaselines, resetLSPService, } = deps;
        ...
        setImmediate(() => {
            void loadLSPConfig(cwd).then((lspConfig) => {
                ...
                if (warmFiles.length > 0) {
                    ...
                    igniteDominantLanguageWarm(cwd, runtime, sessionGeneration, dbg).catch((err) => dbg(`session_start lsp-warm: unhandled dominant error: ${err}`));
                }
            });
        });
        phase("lsp-config");
