...
import { highImportCouplingRule, noBooleanParamsRule, noComplexConditionalsRule, } from "./rules/quality-rules.js";
import { commentedCredentialsRule, commentedOutCodeRule, corsWildcardRule, duplicateStringLiteralRule, dynamicRegexpRule, functionInLoopRule, maxSwitchCasesRule, } from "./rules/sonar-rules.js";
...
const SEMGREP_SUPPORTED_KINDS = new Set([
    "csharp",
    "css",
    "cxx",
    "dart",
...
function withSemgrepGroup(kind, groups, ctx) {
    if (!SEMGREP_SUPPORTED_KINDS.has(kind))
        return groups;
...
function withPrimaryPolicyGroup(kind, groups, pi) {
    ...
    const normalizedGroups = lspEnabled
        ...
        : groups
            .map((group) => {
            const runnerIds = group.runnerIds.filter((id) => id !== "lsp" && id !== "ts-lsp");
...
const CASCADE_NEIGHBOUR_BUDGET = Math.max(MAX_FILES, Number.parseInt(process.env.PI_LENS_CASCADE_NEIGHBOUR_BUDGET ?? "40", 10) || 40);
...
export async function computeCascadeForFile(filePath, cwd, options = {}) {
    ...
    if (hasBlockers) {
        ...
        return { filePath, result: undefined, neighborCount: 0, diagnosticCount: 0, skipReason: "blockers" };
    }
    const fileKind = detectFileKind(filePath);
    if (!fileKind) {
        logCascade({ phase: "cascade_skip", filePath, reason: "non_code_file" });
        return { filePath, result: undefined, neighborCount: 0, diagnosticCount: 0, skipReason: "non_code" };
    }
